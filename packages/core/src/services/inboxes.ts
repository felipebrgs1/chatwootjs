...
function toApiWorkingHour(row: typeof workingHours.$inferSelect): ApiWorkingHour {
  return {
    id: row.id,
    day_of_week: row.dayOfWeek ?? 0,
    closed_all_day: row.closedAllDay ?? false,
    open_all_day: row.openAllDay ?? false,
...
export async function toApiInbox(inbox: Inbox): Promise<ApiInbox> {
  ...
  return {
    ...
    enable_auto_assignment: inbox.enableAutoAssignment ?? false,
    greeting_enabled: inbox.greetingEnabled ?? false,
    greeting_message: inbox.greetingMessage,
    working_hours_enabled: inbox.workingHoursEnabled ?? false,
    out_of_office_message: inbox.outOfOfficeMessage,
    timezone: inbox.timezone ?? "UTC",
    enable_email_collect: inbox.enableEmailCollect ?? false,
    csat_survey_enabled: inbox.csatSurveyEnabled ?? false,
    allow_messages_after_resolved: inbox.allowMessagesAfterResolved ?? false,
    lock_to_single_conversation: inbox.lockToSingleConversation ?? false,
    sender_name_type: inbox.senderNameType ?? "friendly",
